...
def main():
    ...
    for category in KEYWORD_CATEGORIES.keys():
        normalized_col = f'{category}_normalized'
        plt.hist([edu_df[edu_df['domain_type'] == dt][normalized_col] for dt in ['HighSchool', 'CollegeGrad']],
                 bins=20, alpha=0.6, label=category.replace('_', ' '))
    plt.xlabel('Normalized Frequency (per 100 words)', fontsize=12, labelpad=10)
    plt.ylabel('Number of Policies', fontsize=12, labelpad=10)
    plt.title('Detailed Distribution of AI Policy Categories: High School vs College',
              pad=20, fontsize=14, fontweight='bold')
    ...
    print(f"\nAnalysis complete! Visualization saved in {results_path}/education_comparison.png and {results_path}/education_comparison_detailed.png")
